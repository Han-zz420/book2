// ============================================================
// 二手书平台后端 API（Cloudflare Pages Functions + D1）
//   路由（相对 /api 前缀，由 functions/api/[[path]].js 统一承载）：
...
// Pages Functions 的 onRequest 处理器，覆盖 /api/* 全部子路径
export async function onRequest(context) {
  const { request, env, params } = context;
  const url = new URL(request.url);
  // params.path 为 [[path]] 捕获的相对路径段数组（如 ["books"]、["books","1"]、["order"]）
  // 兼容字符串或数组两种形式
  ...
  // ---- 书籍列表（含实时库存） ----
  if (request.method === "GET" && sub === "books") {
    try {
  ...
  // ---- 单本书实时库存 ----
  const bookMatch = sub.match(/^books\/(\d+)$/);
  ...
  // ---- 我的订单：按手机号/微信号查询历史订单 ----
  if (request.method === "GET" && sub === "orders") {
    try {
  ...
  // ---- 按订单号查询单个订单 ----
  const orderMatch = sub.match(/^orders\/([A-Za-z0-9]+)$/);
  ...
  // ---- 下单 ----
  if (request.method === "POST" && sub === "order") {
    let body;
    try {
